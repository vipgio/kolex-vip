...
const http = axiosRateLimit(axios.create(), { maxRequests: 100, perMilliseconds: 60000 });

const CardGallery = React.memo(
	({ cards, user, filter, selectedCollection, owned, categoryId }) => {
											...
											.map((item) => {
												...
												return {
													...
													title: selectedCards.find((o) => o.id === item.cardTemplateId)
														.title,
													templateUUID: selectedCards.find(
														(o) => o.id === item.cardTemplateId
													).uuid,
