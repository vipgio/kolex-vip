	...
	return (
		<>
			<div className='pointer fixed inset-0 z-10 flex flex-col items-center justify-center bg-black/70'>
				<div className='pointer absolute inset-0 z-20 my-8 mx-8 box-content flex flex-col overflow-hidden rounded-md bg-gray-900 sm:mx-24'>
					<div
						className='relative flex h-12 w-full items-center border-b border-b-white/10 bg-gray-800' /*modal header*/
					>
						<h1 className='mx-auto py-2 text-3xl text-gray-200'>{packTemplate.name}</h1>
						<button
...
const Footer = ({ page, setPage, disabled }) => {
	return (
		<div className='relative bottom-0 z-20 mt-auto mb-1 flex h-10 border-t border-gray-300 pt-2 text-gray-300'>
			{page === 1 ? (
				<div
					className={`${
						disabled
